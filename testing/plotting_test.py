"""
Storing functions used in plotting the digits data.
"""
...
def normal_vs_anomaly_2D_all(
    ...
    job_id=None,
    y=None,
    xlabel=r"$\overline{h_{i,1}}$",
    ylabel=r"$\overline{h_{i,2}}$",
):
    ...
    for i in range(nr_trials):
        ...
        # meshgrid for plots
        xx1, yy1 = np.meshgrid(
            np.linspace(xmin - margin_x, xmax + margin_x, 500),
        ...
        ax.clabel(
            ...
            inline=True,
            fmt="  Hyperplane  ",
            manual=[(xmin + 2 * margin_x, ymin + 4 * margin_y)],
            fontsize=8,
        )
        ...
        color = [
            ...
            "m1",
            "k1",
        ]  # plt.cm.(np.linspace(0,1,2*len(data_dict))) # since I won't put 10 digits in one plot
        ...
        plt.savefig(
            f"{file_name}/trial_{i}_all_no_title",
        ...
        plt.title(
            r"$\overline{h_i}$"
            + f" Test States {'- Job '+str(job_id) if job_id is not None else ''}- Trial_{i}"
        )
        plt.savefig(
            f"{file_name}/trial_{i}_all",
            bbox_extra_artists=(lgd,),
            bbox_inches="tight",
            dpi=300,
        )
...
def normal_vs_anomaly_2D_qg(
    ...
    for i, (f1, f2) in enumerate(feature_combinations):
        for j, d in enumerate(data):  # quark and gluon jets
            ...
            axs[i].scatter(
                ...
                alpha=0.5,
                zorder=3,
                label=labels[j] + " - Normal",
            )  # , edgecolors="k", marker=markers[j]
    ...
    plt.subplots_adjust(
        left=0.1, bottom=0.1, right=0.87, top=0.86, wspace=0.3, hspace=0.4
    )

    # save figure without and with title #TODO title
    plt.savefig(
        f"{out_dir}/{'trial_'+str(trial)+'_' if type(trial) == int else ''}{splittings}_no_title",
        bbox_extra_artists=(lgd,),
        bbox_inches="tight",
        dpi=300,
    )
    ...
    plt.savefig(
        f"{out_dir}/{'trial_'+str(trial)+'_' if type(trial) == int else ''}{splittings}",
        bbox_extra_artists=(lgd,),
        bbox_inches="tight",
        dpi=300,
    )
...
def sk_train_plot(model, X1, y=None, fit=False, ax=plt):
    ...
    # meshgrid for plots
    xx1, yy1 = np.meshgrid(
        np.linspace(X1[:, 0].min(), X1[:, 0].max(), 500),
...
def ROC_curve_digits(digits_data_normal, digits_data_anomaly, trials, job_id):
    ...
    for i in range(len(trials)):
        ...
        data_list = [{"data": item, "y_true": 1} for item in digits_data_normal] + [
            {"data": item, "y_true": 0} for item in digits_data_anomaly
        ]
...
def get_dr_kt(
    branches, features=["sigJetRecur_dr12", "sigJetRecur_jetpt", "sigJetRecur_z"]
):
...
def lund_planes(
    ...
    for dataset in (normal, anomalous):
        ...
        y = np.log(flat_kt)
        histograms.append(
            np.histogram2d(x.to_numpy(), y.to_numpy(), range=[[0, 8], [-7, 5]], bins=20)
        )
    ...
    fig, axs = plt.subplots(
        1, 2, sharex=False, sharey=False, figsize=(12, 5), dpi=160
    ...
    for i, (ax, histogram) in enumerate(zip(axs.flat, histograms)):
        ...
        for z in zs:
            ax.plot(
                [np.log(0.4 / rg) for rg in rgs],
                [np.log(pt * z * rg) for rg in rgs],
                color="grey",
            )
        # plot Lund
        ...
        ax.set_yticks(
            np.arange(min(yedges + 1), max(yedges), 2.0)
            if 0 not in yedges
            else np.arange(min(yedges), max(yedges), 2.0)
        )

    # colorbar & adjust
    fig.subplots_adjust(
        left=0.12, right=0.92, top=0.9, bottom=0.18, hspace=0.3, wspace=0.4
    )
    im_ratio = H.shape[0] / H.shape[1]
    cbar = fig.colorbar(
        im, ax=axs.ravel().tolist(), fraction=0.046 * im_ratio, pad=0.04, shrink=0.99
    )  #  ,shrink=0.65
    cbar.set_label("Count")

    # savefig
    plt.savefig(
        f"{out_dir}/{'trial_'+str(trial)+'_' if type(trial) == int else ''}_{info}"
    )
    plt.close("all")
...
def lund_planes_anomalies_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, job_id, trial=None, R=0.4
):
    """Normal data vs anomaly data, next to each other, difference, other?"""
    ...
    labels = [
        "Normal Gluon Data",
        "Anomalous Gluon Data",
        "Normal Quark Data",
        "Anomalous Quark Data",
    ]
    ...
    fig, axs = plt.subplots(
        2, 2, sharex=False, sharey=False, figsize=(12, 10), dpi=160
    )  # , gridspec_kw={'width_ratios': [1, 1.5]})
    fig.patch.set_facecolor("white")
    ...
    # colorbar & adjust
    fig.subplots_adjust(
        left=0.05, right=0.9, top=0.95, bottom=0.1, hspace=0.4, wspace=0.2
    )
    im_ratio = H.shape[0] / H.shape[1]
    cbar = fig.colorbar(
        im,
    ...
    # savefig
    plt.savefig(
        f"{out_dir}/{'trial_'+str(trial)+'_' if type(trial) == int else ''}_anomalies_qg"
    )
    plt.close("all")
