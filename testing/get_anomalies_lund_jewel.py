"""
Get anomalies and make lund planes comparing jewel sets
in vacuum and with a simple qgp.
"""
...
jets_recur_simple, _ = load_n_filter_data(
    file_name_simple, kt_cut=kt_cut, dr_cut=dr_cut
)
_, _, split_test_data_recur_simple = train_dev_test_split(
    jets_recur_simple, split=[0.7, 0.1]
)

# make same size of 3882 for current sets
split_test_data_recur_vac = split_test_data_recur_vac[
    : len(split_test_data_recur_simple)
]
print(
    f"Vac dataset cut down to size {len(split_test_data_recur_vac)}, the size of simple test set"
)
print("Loading data complete")

# lund plane vac versus simple
lund_planes(
    split_test_data_recur_vac,
    split_test_data_recur_simple,
    labels=["Without QGP", "With QGP"],
    info="quenchedness",
)
...
for i, job_id in enumerate(job_ids):
    ...
    # get anomalies
    _, jets_index_tracker_vac, classification_tracker_vac = get_anomalies(
        split_test_data_recur_vac,
        job_id,
        trials,
        file_name_vac,
        jet_info=type_jets + " Vac",
    )
    _, jets_index_tracker_simple, classification_tracker_simple = get_anomalies(
        split_test_data_recur_simple,
        job_id,
        trials,
        file_name_simple,
        jet_info=type_jets + " Simple",
    )
    ...
    for num in [num] if num is not None else range(len(trials)):
        ...
        for feature in [na.recur_jetpt, na.recur_dr, na.recur_z]:
            ...
            try:
                data = [
                    [ak.firsts(normal_vac[feature]), ak.firsts(anomaly_vac[feature])],
                    [
                        ak.firsts(normal_simple[feature]),
                        ak.firsts(anomaly_simple[feature]),
                    ],
                ]
                ...
                stacked_plot_sided_old(
                    data,
                    title,
                    x_label,
                    out_file,
                    titles=["Unquenched Jets", "Quenched Jets"],
                )
            except ValueError:
                print(
                    f"Either no normal or no anomalous data for {job_id} trial {num}!"
                )
                try:
