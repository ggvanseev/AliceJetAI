"""
As in /functions/ folder, but only for scripts in /testing/.
"""
...
def normalize_sample(sample):
    ...
    sample[:, 1] = np.rint(
        (sample[:, 1]) / (d_dominant) * 100
    )  # - y_min * 0.5 * (1 + dy / d_dominant)
        ...
        for line in f.readlines():
            ...
            if sample_flag:  # if currently writing
                ...
                if digit != -1:  # check if not the first sample (or wrong digit)
                    ...
                    data_dict[digit].append(
                        sample.tolist()
                    )  # append numpied/list? sample .tolist
                    # check if dx > dy
