...
information on each run plus notes are stored in storing_results/run_info.p

first pooling a.k.a last reversed pooling:
...
Dimensions for Vision Architectures. To appear in Proc. of the 30th
...
out_file = ""  # if specifically using previously created  sample, otherwise leave empty
...
kt_cut = None  # for dataset, splittings e.g. kt > 1.0 GeV, assign None if not using
dr_cut = None  # for dataset, splittings e.g. dr > 0.05
multicore_flag = (
    False  # if more cores available: using SparkTrials or Trials, turn off for debuging
)
...
# some extra notes for this run
notes = "regular training Jewel simple, lr=e-2, nu = 0.5, hidden dim = 3"
###-----------------------------------------------------------------------------###
    ...
    [
        {
            ...
            "pooling": hp.choice(
                "pooling", ["last_reversed"]
            ),  # "mean", "last" , "last_reversed"
        }
    ],
)
