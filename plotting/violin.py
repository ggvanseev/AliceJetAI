...
def violin_plots(
    df, min_val, min_df, parameters, job_ids, test_param="loss", yscale="linear"
):
    ...
    if df.empty:
        ...
        for parameter in parameters:
            ...
            for p_val in min_df[p_name].unique():
                ...
                minimum = ax2.scatter(
                    int(
                        [lab.get_text() for lab in ax2.get_xticklabels()].index(
                            str(p_val)
                        )
                    ),
                    min_val,
            ...
            if parameter != "variables":
                ...
                labels = [
                    [
                        variable_names["sigJetRecur_" + l].replace("//", "/")
                        for l in "".join(
                            [i for i in lab.get_text() if i.isalnum()]
                        ).split("sigJetRecur")[1:]
                    ]
