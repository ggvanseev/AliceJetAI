...
def cost_condition_plot(result: dict, title_plot: str, out_file: str):
    ...
    final_cost = ax1.scatter(
        len(track_cost[1:]) - 1,
        track_cost[1:][-1],
        color="k",
        zorder=3,
        label=f"Final Cost: {track_cost[1:][-1]:.2E}",
    )
    final_cost.set_clip_on(False)  # so the marker can overlap the axis
    cost = ax1.plot(
        [x for x in range(len(track_cost[1:]))],
        track_cost[1:],
        color="red",
        alpha=0.85,
        zorder=2,
    ...
    plt.legend(
        legend, labels, loc=1
    )  # , borderaxespad=0.1 # put on ax2 since cost is more important -> legend will follow cost line
...
def cost_auc_plot(result: dict, title_plot: str, out_file: str):
    ...
    ax1 = ax.twinx()
    roc = ax1.plot(
        track_roc_auc, zorder=1, alpha=0.85, label="ROC AUC\nOf Test Dataset"
    )
    roc[0].remove()  # remove from this axis
    ax.add_artist(roc[0])  # add to axis 1 to plot behind cost function
    final_roc = ax1.scatter(
        len(track_roc_auc[1:]) - 1,
        track_roc_auc[1:][-1],
        color="k",
        zorder=3,
        label=f"Final ROC AUC: {track_roc_auc[1:][-1]:.4f}",
    )
    ...
    mean_auc = sum(track_roc_auc[epoch_halfway + 1 :]) / len(
        track_roc_auc[epoch_halfway + 1 :]
    )
    if mean_auc > 0.63:
        ...
        if len([x for x in track_cost if x >= max_cost * 0.5]) > len(
            [x for x in track_cost if x < max_cost * 0.5]
        ):
            bbox_loc = (0, 0)
            loc = 3
    ...
    labels = [l.get_label() for l in legend]
    ax1.legend(
        legend, labels, loc=loc, bbox_to_anchor=bbox_loc
    )  # , borderaxespad=0.1 # put on ax2 since cost is more important -> legend will follow cost line
    plt.tight_layout()
...
def cost_condition_plots(trials: dict, job_id):
    ...
    for i, trial in enumerate(trials["_trials"]):
        ...
        if ("roc_auc" in result["cost_data"]) and (result["cost_data"]["roc_auc"]):
            cost_auc_plot(
                result, title_plot, out_file=out_dir + "/cost_auc_" + f"trial_{i}"
            )
        out_txt_trial += "\n\n"
        print(out_txt_trial)
