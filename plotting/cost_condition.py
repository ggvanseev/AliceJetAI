...
def cost_condition_plot(result: dict, title_plot: str, out_file: str):
    ...
    ax1.plot(
        ...
        linewidth=1.3,
        alpha=0.85,
        label=r"Cost Condition: $(\kappa(\mathbf{ \theta }_{k+1}, \mathbf{ \alpha }_{k+1}) - \kappa(\mathbf{ \theta }_k, \mathbf{ \alpha }_k))^2$",
    )
    ...
    ax2.plot(
        ...
        linewidth=1.3,
        alpha=0.85,
        label=r"Cost: $\kappa( \mathbf{ \theta }_{k+1}, \mathbf{ \alpha }_{k+1})$",
    )
...
def cost_condition_plots(trials: dict, job_id):
    ...
    for i, trial in enumerate(trials["_trials"]):
        ...
        # generate the plot
        fig = cost_condition_plot(
            result, title_plot, out_file=out_dir + "/" f"trial_{i}.png"
        )
        if fig == -1:
            break
