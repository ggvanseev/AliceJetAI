"""
Contains all the ROC curve functions used in plotting.
"""
...
def ROC_plot_curve(
    ...
    false_neg_under_th = np.count_nonzero(
        results[(results[:, 0] == 1) & (results[:, 1] < 0)]
    )
    true_pos_under_th = np.count_nonzero(
        results[(results[:, 0] == 1) & (results[:, 1] >= 0)]
    )

    true_neg_under_th = np.count_nonzero(
        results[(results[:, 0] == 0) & (results[:, 1] < 0)]
    )
    false_pos_under_th = np.count_nonzero(
        results[(results[:, 0] == 0) & (results[:, 1] >= 0)]
    )
...
def ROC_curve_qg(g_recur, q_recur, trials, job_id):
    ...
    for i in range(len(trials)):
        ...
        data_list = [
            {**item, **y} for item, y in zip(g_recur.to_list(), g_true.to_list())
        ] + [{**item, **y} for item, y in zip(q_recur.to_list(), q_true.to_list())]
        ...
        _, auc = ROC_plot_curve(
            y_true,
            y_predict,
            plot_title,
            out_file,
            xlabel="Normal Fraction Quarks (FPR)",
            ylabel="Normal Fraction Gluons (TPR)",
        )
...
def ROC_feature_curve_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, samples=None
):
    ...
    for feature in features:
        ...
        for i in np.linspace(minimum, maximum, 100):
            false_neg_under_th = ak.count_nonzero(
                false_neg[false_neg < i]
            )  # false negative
            true_pos_under_th = ak.count_nonzero(
                true_pos[true_pos < i]
            )  # true positive

            true_neg_under_th = ak.count_nonzero(
                true_neg[true_neg < i]
            )  # true negative
            false_pos_under_th = ak.count_nonzero(
...
def ROC_anomalies_hand_cut_lstm(g_recur, q_recur, job_id, trials):
    ...
    for i in range(len(trials)):
        ...
        for j in range(dimensions):
            ...
            data_list = [
                {**item, "y_predict": y} for item, y in zip(data_list, y_predict)
            ]
            y_true = [d["y_true"] for d in data_list]
            ...
            _, auc = ROC_plot_curve(
                ...
                plot_title,
                out_file,
                xlabel="Normal Fraction Quarks",
                ylabel="Normal Fraction Gluons",
            )
            aucs.append(auc)
