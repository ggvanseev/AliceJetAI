...
def svm_boundary_train_plot(
    model, X1, out_file, title=r"$\overline{h_i}$", y=None, fit=False, ax=plt
):
    ...
    ax.scatter(
        ...
        c=y[model.support_] if y else model.predict(model.support_vectors_),
        cmap=plt.cm.viridis,
        lw=1,
        edgecolors="k",
...
def svm_boundary_plots(trials: dict, job_id, train_data, labels=None, fit=False):
    ...
    for i, trial in enumerate(trials["_trials"]):
        ...
        train_batches, track_jets_in_batch, _, _ = branch_filler(
            train_data, h_parm["batch_size"], n_features=n_features
        )
        ...
        train_loader = lstm_data_prep(
            data=train_batches,
            scaler=scaler,
            batch_size=h_parm["batch_size"],
            fit_flag=False,
        )

        # get h_bar states
        h_bar_list, _, _ = calc_lstm_results(
            lstm, len(train_batches[0]), train_loader, track_jets_in_batch
        )
        h_bar_list_np = h_bar_list_to_numpy(h_bar_list, torch.device("cpu"))

        # generate the plot
        fig = svm_boundary_train_plot(
            ocsvm,
            h_bar_list_np,
            out_file=out_dir + "/" f"trial_{i}.png",
            title=r"$\overline{h_i}$" + f" train states - trial_{i}",
            y=labels,
            fit=fit,
        )
        if fig == -1:
