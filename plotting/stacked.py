"""
Contains all the functions used for stacked plots in plotting.
"""
...
def stacked_plot(
    ...
    x_label,
    out_file,
    labels=["Gluon - Normal", "Gluon - Anomaly", "Quark - Normal", "Quark - Anomaly"],
):
...
def stacked_plot_sided(
    data,
    fig_title,
    x_label,
    out_file,
    titles=["Gluon Jets", "Quark Jets"],
    ...
    for i, title in enumerate(titles):
        ...
        anom_fraction = (hist[0][1] - hist[0][0]) / hist[0][
            1
        ]  # anomalies ( = (total - normal) ) / total
        anom_plot = anom_ax.errorbar(
            (hist[1][:-1] + hist[1][1:]) / 2,
    ...
    fig.subplots_adjust(
        left=0.1, right=1.0, top=0.85, bottom=0.12, hspace=0.3, wspace=0.1
    )
    lgd = fig.legend(
        legend,
        labels,
    ...
    # save plot
    plt.savefig(
        out_file + "_no_title", bbox_extra_artists=(lgd,), bbox_inches="tight", dpi=160
    )
    plt.title(fig_title)
...
def stacked_plots_first_entries_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    ...
    for feature in features:
        try:
            data = [
                ...
                ak.firsts(g_anomaly[feature]),
                ak.firsts(q_normal[feature]),
                ak.firsts(q_anomaly[feature]),
            ]
        except ValueError:
...
def stacked_plots_last_entries_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
...
def stacked_plots_normalised_first_entries_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
    ...
    for feature in features:
        try:
            data = [
                ...
                [ak.mean(x) for x in g_anomaly[feature]],
                [ak.mean(x) for x in q_normal[feature]],
                [ak.mean(x) for x in q_anomaly[feature]],
            ]
        except ValueError:
            print(f"Either no normal or no anomalous data for {job_id} trial {trial}!")
            return -1
...
def stacked_plots_splittings_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
    ...
    for feature in features:
        data = [
            [ak.count(x) for x in g_normal[feature]],
            [ak.count(x) for x in g_anomaly[feature]],
            [ak.count(x) for x in q_normal[feature]],
            [ak.count(x) for x in q_anomaly[feature]],
        ]
...
def stacked_plots_all_splits_qg(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
    ...
    for feature in features:
        try:
            data = [
                ...
                ak.flatten(g_anomaly[feature]),
                ak.flatten(q_normal[feature]),
                ak.flatten(q_anomaly[feature]),
            ]
        except ValueError:
            print(f"Either no normal or no anomalous data for {job_id} trial {trial}!")
            return -1
...
# side - by - side
def stacked_plots_first_entries_qg_sided(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
...
def stacked_plots_last_entries_qg_sided(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
...
def stacked_plots_mean_qg_sided(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
...
def stacked_plots_splittings_qg_sided(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
...
def stacked_plots_normalised_first_entries_qg_sided(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
...
def stacked_plots_all_splits_qg_sided(
    g_anomaly, g_normal, q_anomaly, q_normal, features, job_id, trial
):
    # store stacked plots in designated directory
    out_dir = f"output/stacked_plots_{job_id}"
