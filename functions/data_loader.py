...
def load_n_filter_data(
    ...
    Args:
        ...
        tree_name (str, optional): Name of the TTree object of the ROOT file. Defaults
                to "jetTreeSig".
        cut (bool, optional): Boolean statement whether to apply eta & p_t cuts. Defaults
                to True.
...
def mix_quark_gluon_samples(
    file_name, jet_branches=None, g_percentage=90, kt_cut=None, dr_cut=None
):
    ...
    # open file and select jet data and recursive jet data
    data = load_n_filter_data_qg(
        file_name, jet_branches=jet_branches, kt_cut=kt_cut, dr_cut=dr_cut
    )

    # calculate g and q bounds
    total = round(
        len(data[0]) / 100
    )  # judge total off of about the total number of gluon jets /100
    ...
    if jet_branches:
        ...
        data = [
            {**sd, **sd_recur}
            for sd, sd_recur in zip(data[2].to_list(), data[0].to_list())
        ][:g_len] + [
            {**sd, **sd_recur}
            for sd, sd_recur in zip(data[3].to_list(), data[1].to_list())
        ][
            :q_len
        ]
        ...
        data = ak.Array(
            [{key: d[key] for key in data[0] if key not in jet_branches} for d in data]
        ), ak.Array([{key: d[key] for key in jet_branches} for d in data])
        ...
        # create dataset from bounds as list
        data = [{**d} for d in data[0].to_list()][:g_len] + [
            {**d} for d in data[1].to_list()
        ][:q_len]
        sd = None
